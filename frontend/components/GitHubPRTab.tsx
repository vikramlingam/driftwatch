      ...
      {ghScanResult && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
            <div className={`card-dark p-4.5 ${theme === 'dark' ? 'bg-[#11141c]' : 'bg-white'}`}>
              <span className="text-[10px] font-semibold uppercase tracking-wider text-[#64748b]">
                Target Repository
              </span>
              <p className={`mt-1 font-mono text-sm font-bold truncate ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>
                {ghScanResult.repo_name}
              </p>
              <span className="text-[10px] text-[#64748b]">Branch: {ghScanResult.default_branch}</span>
            </div>
            ...
            >
              ...
              <p className={`mt-1 text-base font-bold ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>
            ...
            >
              ...
              <span className="text-[10px] text-[#64748b]">AST symbol analysis</span>
            </div>

            <div
              className={`rounded-xl border p-4.5 transition ${
                ghScanResult.impact_matches.length > 0
                  ? theme === 'dark' ? 'border-amber-500/30 bg-[#11141c]' : 'border-amber-300 bg-white shadow-sm'
                  : theme === 'dark' ? 'border-emerald-500/30 bg-[#11141c]' : 'border-emerald-300 bg-white shadow-sm'
              }`}
            >
              <span className="text-[10px] font-semibold uppercase tracking-wider text-[#64748b]">
                Breaking API Drift
              ...
              <div className={`mt-4 divide-y ${theme === 'dark' ? 'divide-[#1e2433]' : 'divide-slate-200'}`}>
                {paginatedGhMatches.map((match, idx) => {
                  ...
                  return (
                    <div
                      key={idx}
                      className={`py-4 transition flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 ${
                        isSelected
                          ? theme === 'dark'
                            ? 'bg-[#161a25]/50 px-3 rounded-md'
                            : 'bg-indigo-50/70 px-3 rounded-md'
                          : ''
                      }`}
                    >
                      ...
                      <button
                        ...
                        className="flex items-center gap-1.5 rounded-md bg-indigo-600 px-4 py-2 text-xs font-semibold text-white transition hover:bg-indigo-500 disabled:opacity-50 whitespace-nowrap self-start lg:self-center"
                      >
          ...
          {/* AI Review Findings & Pull Request Studio Cockpit */}
          {llmReviewResult && selectedGhMatch && (
            <div
              className={`card-dark p-6 space-y-6 border-indigo-500/40 ${
                theme === 'dark' ? 'bg-[#10131d]' : 'bg-white'
              }`}
            >
              <div
              ...
              >
                <div>
                  ...
                  <p className={`text-xs mt-0.5 ${theme === 'dark' ? 'text-[#94a3b8]' : 'text-slate-600'}`}>
                    Target: <code className={`font-mono ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>{selectedGhMatch.file_path}</code> (Powered by {llmReviewResult.model_used})
                  </p>
                </div>
              ...
              <div>
                <div className="flex items-center justify-between pb-2">
                  ...
                  <button
                    onClick={() => copySnippet(llmReviewResult.unified_diff, 'llm-diff')}
                    className={`flex items-center gap-1 text-xs ${
                      theme === 'dark' ? 'text-[#94a3b8] hover:text-white' : 'text-slate-600 hover:text-slate-900'
                    }`}
                  >
                    <Copy size={13} />
                    <span>{copiedId === 'llm-diff' ? 'Copied Diff' : 'Copy Unified Diff'}</span>
                  </button>
                </div>
